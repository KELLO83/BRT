...
class YOLODetector:
    ...
    def run(self):
        ...
        for index , i in tqdm(enumerate(self.distortion_list),total=total_d):
            ...
            # SAVE_DIR = f'FISH_IMAGE_{number}'
            # os.makedirs(SAVE_DIR , exist_ok=True)
            # name = os.path.basename(i)
            # cv2.imwrite(os.path.join(SAVE_DIR , name) , undisort_image)
            

            undisort_result = self.model(undisort_image , verbose = False , classes = 0)
            undisort_result_copy = copy.deepcopy(undisort_result)

            distort_box_image , dis_count , is_seat_dis , b1 = generat_box_module.call_generate_box_final(undisort_result , undisort_image , number=self.number , image_name = i)
            #undistort_box_image , und_count , is_seat_und , b2 = generat_box_module.call_generate_box_final(undisort_result_copy , undisort_image , number=self.number)
        ...
        # print(generat_box_module.sit_count_old)
        # print('==================================================')
        # print("정답 : \n",generat_box_module.sit_count)
        # print('==================================================')
...
if __name__ == "__main__":
    ...
    #name = 'cv2_diff_test/removed_raw_seat/8'

    number = name.split('/')[-1]
    distort_images = natsorted(glob(os.path.join(f'{name}','*.jpg')))
    
    #distort_images = natsorted(glob(os.path.join(f"empty_front",'**',"*.jpg"),recursive=True))
